...
export default function EditItem() {
    ...
    useEffect(() => {
        if (itemData) {
            setIsServiceMode(itemData.type === 'service');
            setFormData({
                id: itemData.id?.toString() || "",
                itemCode: itemData.itemCode?.toString() || "",
                itemName: itemData.itemName?.toString() || "",
                hsnCode: itemData.hsnCode?.toString() || "",
                unit: itemData.unit?.toString() || "",
                serviceUnit: itemData.serviceUnit?.toString() || "",
                purchasePrice: itemData.purchasePrice?.toString() || "",
                sellingPrice: itemData.sellingPrice?.toString() || "",
                mrp: itemData.mrp?.toString() || "",
                openingStock: itemData.openingStock?.toString() || "",
                openingStockValue: itemData.openingStockValue?.toString() || "",
                minStock: itemData.minStock?.toString() || "",
                description: itemData.description?.toString() || "",
                taxStatus: itemData.taxStatus?.toString() || "taxable",
                gstRate: itemData.gstRate?.toString() || "18",
            });
        }
    }, [itemData]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result.status) {
                toast({
                    title: "Success",
                    description: `${isServiceMode ? "Service" : "Item"} updated successfully!`,
                });
                navigate("/item");
            } else {
                toast({
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="flex justify-end gap-3 pt-4">
                                ...
                                <Button type="submit" disabled={isLoading}>
                                    {isLoading ? "Updating..." : `Update ${isServiceMode ? "Service" : "Item"}`}
                                </Button>
                            </div>
                        </form>
                    </CardContent>
